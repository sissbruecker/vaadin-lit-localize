...
import { localizeUpload } from "../src/directives.js";
...
describe("localizeLoginForm", () => {
  ...
  before(async () => {
    ...
    upload = await fixture(html`
      <vaadin-upload ${localizeUpload()}></vaadin-upload>
    `);
  });
  ...
  it("should use default i18n values", () => {
    ...
    expect(upload.i18n.error.incorrectFileType).to.equal("Incorrect File Type.");
    ...
    expect(upload.i18n.uploading.error.unexpectedServerError).to.equal("Upload failed due to server error");
    expect(upload.i18n.uploading.remainingTime.prefix).to.equal("remaining time: ");
    expect(upload.i18n.uploading.status.connecting).to.equal("Connecting...");
    expect(upload.i18n.uploading.status.processing).to.equal("Processing File...");
  });
  ...
  it("should use localized i18n values when changing locale", async () => {
    ...
    expect(upload.i18n.error.incorrectFileType).to.equal("Ungültige Dateiart");
    expect(upload.i18n.file.remove).to.equal("Entfernen");
    expect(upload.i18n.file.start).to.equal("Start");
    expect(upload.i18n.units.size[0]).to.equal("B");
    expect(upload.i18n.units.size[5]).to.equal("PB");
    expect(upload.i18n.uploading.error.unexpectedServerError).to.equal("Upload fehlgeschlagen, bitte versuche es später noch einmal");
    expect(upload.i18n.uploading.remainingTime.prefix).to.equal("verbleibende Zeit: ");
    expect(upload.i18n.uploading.status.connecting).to.equal("Verbinden");
    expect(upload.i18n.uploading.status.processing).to.equal("Verarbeite Datei...");
  });
});
